...
CREATE TABLE user_stock_rules (
    ...
    notify_on_match BOOLEAN NOT NULL DEFAULT TRUE,    -- 命中后是否写通知（false 仅用作筛选记录）
...
```
### 每日资金流表 (stock_money_flow_daily)

```sql
CREATE TABLE stock_money_flow_daily (
    symbol      VARCHAR(10)    NOT NULL,
    trade_date  DATE           NOT NULL,
    in_amount   DECIMAL(20,4)  NOT NULL DEFAULT 0,  -- 主力流入(元)
    out_amount  DECIMAL(20,4)  NOT NULL DEFAULT 0,  -- 主力流出(元)
    net_amount  DECIMAL(20,4)  NOT NULL DEFAULT 0,  -- 主力净流入(元)
    super_net   DECIMAL(20,4)  NOT NULL DEFAULT 0,  -- 超大单净额
    large_net   DECIMAL(20,4)  NOT NULL DEFAULT 0,  -- 大单净额
    medium_net  DECIMAL(20,4)  NOT NULL DEFAULT 0,  -- 中单净额
    small_net   DECIMAL(20,4)  NOT NULL DEFAULT 0,  -- 小单净额
    PRIMARY KEY (symbol, trade_date)
);
```

来源：东财 `push2his.eastmoney.com/api/qt/stock/fflow/daykline/get`，由 fetcher 周期抓取，写入后供 `stock_history_mv` 联表使用。

`stock_history_mv` 在原有 `in_amount/out_amount/net_amount` 之外补充 `super_net/large_net/medium_net/small_net` 列。
...
CREATE TABLE notifications (
    id         SERIAL PRIMARY KEY,
    user_id    VARCHAR(100) NOT NULL,        -- users.id (UUID 字符串形态)
    rule_id    INT,                          -- 关联 user_stock_rules.id
    rule_name  VARCHAR(100),                 -- 冗余存规则名，避免历史通知随规则删除丢失
    symbol     VARCHAR(10)  NOT NULL,        -- 股票代码
    stock_name VARCHAR(50),                  -- 冗余股票名（与规则触发时刻快照对齐）
...
去重粒度：`(user_id, rule_id, symbol, 当日)`。命中后当日内同一 `(user, rule, symbol)` 不重复写入。

## Schema 迁移 (idempotent)

`deploy/db/01_init.sql` 末尾保留以下 idempotent ALTER，重复启动不会报错：

```sql
ALTER TABLE user_stock_rules ADD COLUMN IF NOT EXISTS notify_on_match BOOLEAN NOT NULL DEFAULT TRUE;
ALTER TABLE stock_basic_info ADD COLUMN IF NOT EXISTS pettm DECIMAL(10,4);
ALTER TABLE stock_basic_info ADD COLUMN IF NOT EXISTS pb    DECIMAL(10,4);
ALTER TABLE stock_history_mv ADD COLUMN IF NOT EXISTS super_net  DECIMAL(20,4) DEFAULT 0;
ALTER TABLE stock_history_mv ADD COLUMN IF NOT EXISTS large_net  DECIMAL(20,4) DEFAULT 0;
ALTER TABLE stock_history_mv ADD COLUMN IF NOT EXISTS medium_net DECIMAL(20,4) DEFAULT 0;
ALTER TABLE stock_history_mv ADD COLUMN IF NOT EXISTS small_net  DECIMAL(20,4) DEFAULT 0;
```

## 默认管理员

`01_init.sql` 会幂等地插入 admin 用户，密码 `admin123`，bcrypt cost=10。**生产部署后请立刻通过 `/api/v1/user/login` 登录并修改密码**（目前 backend 没有"改密码"端点，需要手动更新 `users.password_hash`）。
