...
def main():
    ...
    #engine = create_engine("postgresql+psycopg2://postgres:pass@localhost:5432/a_stock")
    ...
    if os.path.exists(industry_map_csv):
        industry_map_df = pd.read_csv(industry_map_csv,  dtype=str)
    ...
    existing_symbols = pd.read_sql("SELECT symbol FROM stock_basic_info", engine)
    #print(existing_symbols['symbol'])  
    symbols_set = set(existing_symbols['symbol'])  
    for idx, row in all_stocks.iterrows():
        symbol = row['证券代码'] if '证券代码' in row else row.get('代码')
        if symbol in symbols_set:
            print("symbol:" + symbol + "has imported")
            continue
        if len(symbol) < 6:
